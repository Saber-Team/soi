...
Task.prototype.resolveImgRelativeUri = function () {
  ...
  imgResources.forEach(function (imgPath) {
    ...
    if (result.length) {
      this.map.resourceMap.Image[imgPath].uri = result[result.length - 1]['to'] + '/' + name;
    }
  });
};
...
Task.prototype.resolveJsRelativeUri = function () {
  ...
  jsResource.forEach(function(jsPath) {
    ...
    _this.map.resourceMap.JS[jsPath]._fileContent
        .replace(/[require|require.async]\s*\(\s*(['"]?)([^"'\)]*)\1\s*\)/gi,
            function(match, $1, $2) {
              // $2 就是匹配出来的依赖路径
 ...
 * @param jsObj
 */
Task.property.addJsWrapper = function (jsObj) {
