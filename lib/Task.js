  ...
  var content = resource.getContent()
      .replace(RE_URL, function($0, $1) {
        ...
        if (inlineResource) {
          ...
          soi.log.warn('Image [' + $1 + '] in file [' + resource.path + '] Not resolved');
        }
...
Task.prototype._compileJS = function(resource) {
  ...
  if (resource.isModule && !resource.isCompiled) {
...
Task.prototype.resolveProductUriInternal = function(resource) {
  ...
  */
  resource.uri = this.rules.getUriByPath(resource.path) + path.parse(resource.path).base;
  //debugger
};
