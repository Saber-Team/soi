...
var ImageResource = require('./imgResource');
var JsResource = require('./jsResource');
var CssResource = require('./cssResource');
var SwfResource = require('./swfResource');
var FontResource = require('./fontResource');
var SvgResource = require('./svgResource');
var classMap = {
  css: CssResource,
  js: JsResource,
  img: ImageResource,
...
// helper private function
Resource.prototype.__createSwfResource__ = function() {
  var ret = utils.getFileHash(this.path, this.encoding);
  var dir = path.dirname(this.path);
  var ext = path.extname(this.path);
  var fname = path.basename(this.path, ext);
...
// helper private function
Resource.prototype.__createFontResource__ = function() {
  var ret = utils.getFileHash(this.path, this.encoding);
  var dir = path.dirname(this.path);
  var ext = path.extname(this.path);
  var fname = path.basename(this.path, ext);
...
Resource.create = function(config) {
  return new classMap[config.type](config);
};
