...
class Compressor {
  ...
  init(task) {
    ...
    this.backUp = this.host.beforeCompile;
    task.onBeforeMethod('beforeCompile', this.exec);
  }

  mangle(selector) {
    let map = this.host.getMap();
    map._cssClassMap = map._cssClassMap ? map._cssClassMap : {};
    ...
    let comp = parts.map(word => {
      if (map._cssClassMap[word]) {
        return map._cssClassMap[word];
      }
      if (cIdx === maxLength) {
        return word;
      }
      map._cssClassMap[word] = cl[cIdx++];
      return map._cssClassMap[word];
    });
  ...
  uninstall() {
    this.host.beforeCompile = this.backUp;
  }
}
