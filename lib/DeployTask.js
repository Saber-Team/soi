...
class DeployTask extends Workflow {
  ...
  flushInternal() {
    ...
    let resPath = node_path.join(task.options.dir, task.options.mapTo, 'resource.json');
    ...
    // 上传打包配置文件
    let pkgPath = node_path.join(task.options.dir, task.options.mapTo, 'packages.json');
