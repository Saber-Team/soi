...
ScriptResolver.prototype.combo = function(scripts) {
  ...
  scripts.forEach(function(filepath) {
    ...
    if (filepath === this.options.module_loader) {
      ...
      if (!mod) {
        soi.log.error('Module located at ', filepath,
      ...
      var us = urls.map(function(url) {
        ...
        // 根据绝对路径拿到注册的package
        var pkg = ResourceTable.getPackageByPath('js', u);
        if (pkg === null) {
          debugger;
        }
        var need = pkg.dist_file;
        // 根据当前遍历包得到打包后当前包的地址
...
ScriptResolver.prototype.write2compress = function(jsFilesArray) {
  ...
  soi.log.info('JS resolver write to file at: ', _jspath_);
