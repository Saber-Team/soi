...
_util.delete = function(fpath) {
  if (fpath && _util.exists(fpath)) {
    ...
    if (stat.isDirectory()) {
      ...
      fs.rmdirSync(fpath);
      soi.log.ok('delete directory [' + fpath + ']: success');
    } else if (stat.isFile()) {
      fs.unlinkSync(fpath);
      soi.log.ok('delete file [' + fpath + ']: success');
    } else {
...
_util.parseUrl = function (url, opt) {
  opt = opt || {};
  url = Url.parse(url);
  opt.host = opt.host || opt.hostname;
...
_util.writeFile = function(fpath, content, options, callback) {
  ...
  options = options || {};
  options.mode = 0777 & (~process.umask());
...
/**
 * 瘦身资源表,瘦身只好保留 id type uri deps（如果有的话）
 * @param map 资源表
 * @return {String}
 */
_util.shimMap = function (map) {
  ...
  _util.map(map.resourceMap, function(type, obj) {
    ...
    soi.util.map(obj, function(key, item) {
      var deps = [], required = item.requiredCSS || item.requiredJS || item.requiredModules;
      m[type][key] = {
        id: item.id,
        uri: item.uri,
        type: item.type
      };
      if (required && soi.util.isArray(required)) {
        deps = required.map(function(requiredItem) {
          var relativePath =  soi.util.relativePath(item.path, requiredItem);
          return map.resourcePathMap[relativePath].id
        });
      }
      deps.length && (m[type][key]['deps'] = deps);
    });
  });
