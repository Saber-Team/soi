    ...
    switch (cmd) {
      case 'deploy':
        try {
          ...
          soi.log.error('Error occurred when reading configure file located at \n  ' +  conf);
          process.exit(1);
        }
        soi.log.info(argv._);
        soi.run('deploy', argv._[0], argv);
        break;
      case 'release':
        try {
          // 执行配置文件中的代码
