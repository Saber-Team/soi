...
/**
 * All source code without id will be automated replaced.
 * @param {String} id
 * @param {Array.<String>} deps
 * @param {Function|Object} factory
 */
function buildCode(id, deps, factory) {
  ...
  codes[currentModulePath[currentModulePath.length - 1]] =
    (id === 'anonymous' ? constants.PREFIX_REQUIRE : constants.PREFIX_DEFINE)
      .replace('%s', id)
      .replace('%a', depsCode) + factoryCode +
    (id === 'anonymous' ? constants.SUFFIX_REQUIRE : constants.SUFFIX_DEFINE);

  return [id, depsCode, factoryCode];
}
 ...
 * @param factory
 */
function retrieveAsync(factory) {
...
var ENV = {
  define: function define(id, deps, factory) {
    ...
    // replace id and dependency
    buildCode(id, deps, factory);

    // deal with factory contains "require.async code"
    retrieveAsync(factory);
