...
var ENV = {
  define: function(id, deps, factory) {
    ...
    // Note: first view js file should create single resource,
    // but which load on demand only registered as package in resource table
    if (ResourceTable.getResource('js', filepath)) {
      return;
    }

    // Module has been registered
    // Note: All js file should registered ModuleManager as a single module.
...
function construct(path) {
  ...
  visited = visited.filter(function(path) {
    ...
    createResource(path);
    /*
    if (currentPkg.defer && ModuleManager.getModuleByPath(path)) {
      return false;
    }*/
    return true;
  });
