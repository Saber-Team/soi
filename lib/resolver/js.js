...
ScriptResolver.prototype.combo = function(scripts) {
  ...
  scripts.forEach(function(filepath) {
    ...
    var us = urls.map(function(url) {
      ...
      // 根据当前遍历包得到打包后当前包的地址
      var cur = ResourceTable.getPackageByPath('js',
        scripts[scripts.length - 1]).dist_file;
      // 计算两个包打包后的相对地址
      return utils.normalizeSysPath(
        path.relative(path.dirname(cur), need));
    }, this);
 ...
 * @private
 */
ScriptResolver.prototype.deorder_ = function() {
